...
public class AiController : ControllerBase
{
    ...
    [Authorize(Policy = AuthorizationPolicies.Staff)]
    [HttpPost("text")]
    public async Task<ActionResult<AiTextResponseDto>> GenerateText(AiTextRequestDto request)
    {
        ...
        var text = await _aiService.GenerateTextAsync(request.Purpose, request.Input.Trim());
    ...
    public async Task<ActionResult<AiTextResponseDto>> GetMyVolunteerSummary()
    {
        ...
        var tasks = await _context.VolunteerTasks
            ...
            {
                task.Title,
                Type = task.VolunteerTaskType != null ? task.VolunteerTaskType.Name : string.Empty,
                Animal = task.Animal != null ? task.Animal.Name : string.Empty,
                Status = task.VolunteerTaskStatus != null ? task.VolunteerTaskStatus.Name : string.Empty,
                task.DueDate
            })
            .ToListAsync();
        ...
        AiFreeTextRequestDto request)
    {
        ...
        var result = await _aiService.GenerateStructuredAsync<AnimalIntakeSuggestionDto>(
            "animal-intake",
            request.Text.Trim());
    ...
    public async Task<ActionResult<AnimalDataCheckDto>> CheckAnimalData(SaveAnimalDto request)
    {
        ...
        var result = await _aiService.GenerateStructuredAsync<AnimalDataCheckDto>(
            "animal-data-check",
            safeInput);
