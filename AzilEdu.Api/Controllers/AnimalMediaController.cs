...
public class AnimalMediaController : ControllerBase
{
    ...
    private static readonly Dictionary<string, (AnimalMediaType Type, string Extension)> AllowedContentTypes =
    ...
    public AnimalMediaController(AzilEduDbContext context, IWebHostEnvironment environment)
    {
        _context = context;
        ...
        [FromForm] string? caption)
    {
        ...
        var imageCount = await _context.AnimalMedia.CountAsync(item =>
            item.AnimalId == animalId && item.MediaType == AnimalMediaType.Image);
        ...
        return CreatedAtAction(nameof(GetMedia), new { animalId }, ToDto(media));
    }
    ...
    public async Task<IActionResult> Delete(int animalId, int mediaId)
    {
        ...
        var physicalPath = Path.Combine(webRoot, "uploads", "animals", Path.GetFileName(media.StoredFileName));
