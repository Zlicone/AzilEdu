...
public class VolunteerTasksController : ControllerBase
{
        ...
        [FromQuery] DateTime? dateTo)
    {
        ...
        {
            query = query.Where(task => task.DueDate >= dateFrom.Value);
        }

        if (dateTo.HasValue)
        {
            query = query.Where(task => task.DueDate <= dateTo.Value);
        }
    ...
    {
        var validationError = await ValidateTaskAsync(request);
    ...
    private async Task<string?> ValidateTaskAsync(SaveVolunteerTaskDto request)
    {
        ...
        var statusExists = await _context.VolunteerTaskStatuses
            .AnyAsync(status => status.Id == request.VolunteerTaskStatusId);
        ...
        var typeExists = await _context.VolunteerTaskTypes
            .AnyAsync(type => type.Id == request.VolunteerTaskTypeId);

        if (!typeExists)
        {
            return "Odabrani tip zadatka ne postoji.";
        }

        var completedStatus = await _context.VolunteerTaskStatuses
            .FirstOrDefaultAsync(status => status.Id == request.VolunteerTaskStatusId);

        var isCompleted = completedStatus != null && completedStatus.Name == "Završeno";

        if (isCompleted && !request.CompletedAt.HasValue)
        {
