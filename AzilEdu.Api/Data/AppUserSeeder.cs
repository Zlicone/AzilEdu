...
public static class AppUserSeeder
{
    public static async Task SeedAsync(AzilEduDbContext db)
    {
        ...
        var employee = new AppUser
        {
            ...
            EmployeeId = await db.Employees.Select(item => (int?)item.Id).FirstOrDefaultAsync()
        };
        ...
        var volunteer = new AppUser
        {
            ...
            VolunteerId = await db.Volunteers.Select(item => (int?)item.Id).FirstOrDefaultAsync()
        };
        ...
        var donor = new AppUser
        {
            ...
            DonorId = await db.Donors.Select(item => (int?)item.Id).FirstOrDefaultAsync()
        };
